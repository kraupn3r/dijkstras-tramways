...
def findstop(latitude, longitude):
...
def transstop(target):
    for count, each in enumerate(TramStop.objects.all()):
        if each.pk == int(target):
            return count


def Setup(startstop, endstop, start_time, start_long, start_lat, end_long, end_lat):
    ...
    if startstop == '':
        first_stop, dist = findstop(float(start_long), float(start_lat))
        timezero = datetime.strptime(
            start_time, '%H:%M:%S') + timedelta(minutes=(int(round(dist * 12, 0))))
        firstwalk = int(round(dist * 12, 0))
        timezero = timezero.strftime('%H:%M:%S')
    else:
        first_stop = startstop
        firstwalk = 0
        timezero = start_time

    if endstop == '':
        last_stop, dist = findstop(float(end_long), float(end_lat))
        lastwalk = int(round(dist * 12, 0))
    else:
        last_stop = endstop
        lastwalk = 0
    ...
    routedict = {}
    current_time = datetime.strptime(timezero, '%H:%M:%S')
    ...
    stime =  (datetime.strptime(routedict[0]['start_time'], '%H:%M') - timedelta(minutes=firstwalk)).strftime(
        '%H:%M')
    etime = (datetime.strptime(routedict[len(routedict) - 1]['end_time'], '%H:%M') + timedelta(minutes=lastwalk)).strftime('%H:%M')
    ttime = datetime.strptime(etime, '%H:%M') - datetime.strptime(stime, '%H:%M')
    tts = datetime.strptime(stime, '%H:%M') - datetime.strptime(start_time, '%H:%M:%S')
    tfn = datetime.strptime(stime, '%H:%M') - datetime.strptime(timezone.localtime().strftime('%H:%M'), '%H:%M')
    routedict[100] = [firstwalk, lastwalk, stime, etime,int(ttime.seconds/60),int(tts.seconds/60),int(tfn.seconds/60)]
