...
<div class="vforicons" v-for="each in content" :key="each.id">
...
<div class="timeline">
 <div ><template v-for="e in content"><div class="divinline" :key="e.id" v-if="e ===content[0] && e.triptime > 0 && e.line === 'walk'"><div class="walkIcon"></div><div class="aftericon">{(e.triptime)} min</div></div></template><span class="timess">{(this.startime)} - {(this.endtime)}</span><template v-for="e in content"><div class="divinline" :key="e.id" v-if="e ===content[content.length-1] && e.triptime > 0  && e.line === 'walk'"><div class="walkIcon"></div><div class="aftericon">{(e.triptime)} min</div></div></template></div>
</div>
</div>
...
<table>
<tbody v-for="each,index in content" :key="each.id">
<tr >
...
<td v-else-if="each.line != 'walk'"> <div class="chapterLineInfo"> <div>Direction: {(each.line_direction)}</div><div>{(each.triptime)} min</div></div><div class="openstops"><div class="clickable " :id="'open'+index" @click="openStops(index)">{(each.stopslist.length)} stops</div><div class="stoplist clickable" @click="closeStops(index)" :id="index" style="display:none;"><div v-for="leach in each.stopslist" :key="leach.id">{(leach[2])} {(leach[1])}</div></div></div></td>
</tr>
<tr class="lastbar" >
...
export default {
  name: "Routeshow", // eslint-disable-line vue/multi-word-component-names
  delimiters: ['{(', ')}'],
  data(){
    return {
  ...
  mounted() {
    ...
    EventBus.$on('gotAPI', () => {
      self.trippTime = this.$store.state.walkvals[4]
      self.starttrip = this.$store.state.walkvals[6]
      document.getElementById('routeshowContainer').style.display = "block";
      self.content = this.$store.state.routeChapterArray;
